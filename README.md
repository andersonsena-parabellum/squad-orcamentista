...
- A base ORSE está `QUARENTENADA` porque o XML disponível apresenta descrições genéricas incompatíveis com uma tabela oficial.
- A base SINAPI está `NAO_VERIFICADA` até que o arquivo bruto oficial e seu hash sejam vinculados ao registro de proveniência.
...
- Enquanto uma fonte usada no orçamento não estiver `LIBERADA`, o pacote não pode receber veredito final de envio.
